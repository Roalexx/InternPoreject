...
class Program
{
    ...
    {   //iqAppId kullanıcı tokenidir yani kimi görmek istediğin değil kimin görmek istediğidir
        string igAppId = "936619743392459";
        ...
        string followingCallerLink = "https://www.instagram.com/api/v1/friendships/" + target1UserId + "/following/?count=200&max_id=0";
        string data;
        ...
        dataInfo = await Functions.AsyncHttpRequest(followingCallerLink, igAppId, cookie);
        data = dataInfo.ToString();
        response1 = JsonConvert.DeserializeObject<InstagramResponse>(data);
        //user 2 nin takiip ettiklerini alalım
        followingCallerLink = "https://www.instagram.com/api/v1/friendships/" + target2UserId + "/following/?count=200&max_id=0";
        dataInfo = await Functions.AsyncHttpRequest(followingCallerLink, igAppId, cookie);
        data = dataInfo.ToString();        
        InstagramResponse response2 = JsonConvert.DeserializeObject<InstagramResponse>(data);

        //user 1 in takipçilerini alalım
        idCallerLink = "https://www.instagram.com/api/v1/friendships/" + target1UserId + "/followers/?count=200&max_id=0";
        dataInfo = await Functions.AsyncHttpRequest(followingCallerLink, igAppId, cookie);
        data = dataInfo.ToString();
        InstagramResponse response3 = JsonConvert.DeserializeObject<InstagramResponse>(data);

        //user 2 nin takipçilerini alalım
        followingCallerLink = "https://www.instagram.com/api/v1/friendships/" + target2UserId + "/followers/?count=200&max_id=0";
        dataInfo = await Functions.AsyncHttpRequest(followingCallerLink, igAppId, cookie);
        data = dataInfo.ToString();
